...
\begin{itemize}
    ...
    \item A motion-adaptive minimal-force term ("task-on-demand") that couples the desired grasp-force floor to the object's motion state, contrasted with a constant-safety-margin baseline.
    \item A joint-torque-capacity-aware extension of the same QP that biases allocation toward the arm with more actuation margin, added as a quadratic term with no increase in problem size or solver cost.
    \item Identification and a first mitigation (activation-blended gain switching) for a coupling failure mode between impedance stiffness and wrench gain during lateral translation.
    \item Validation on a dual xArm7 platform (mc\_rtc).
\end{itemize}
