...
void DualArmControl::ImpactAware(){
       ...
       // Step 4 Vincolo `redittivo nel QP
}
...
void DualArmControl::entryStateCollaborative(){
       ...
       collabSubState_ = CollabSubState::APPROACH_RAMP;
       stateTimer_ = 0.0;
       mc_rtc::log::info("[FSM] BUILDING GRASP");
       // Stabilize the end effector to the last pose of the indipendent phase
...
void DualArmControl::stateCollaborative(){
       ...
       {
              case CollabSubState::APPROACH_RAMP:{
              ...
              rightImpedanceTask_-> targetWrench(sva::ForceVecd(Eigen::Vector3d::Zero(),Eigen::Vector3d(0.0, 0.0, force)));
                     ...
                     auto vl = robots().robot(leftRobotIndex_).bodyVelW(eeName_);
auto vr = robots().robot(rightRobotIndex_).bodyVelW(eeName_);
    ...
    vr.linear().norm()
              ...
              case CollabSubState::BUILD_GRASP:{                
                  ...
                  auto grasp = buildGraspFrame();
                     ...
                     x_0_objectWaypoint1_ = sva::PTransformd(Eigen::Quaterniond(grasp.object.rotation()),Eigen::Vector3d(0.50, 0.0, 0.08));
              ...
              // =========================================================================
              // 1. ALLOCAZIONE E ASSEGNAZIONE DEI GUADAGNI (Configurazione Asimmetrica)
              // =========================================================================
              ...
              // =========================================================================
              // Calcola la forza interna corrente proiettandola nello spazio nullo di Grasp
                     updateContactForces();
              // Definizione delle soglie di forza per la macchina a stati interna
                     double targetForceThreshold = gains.lambda_desired * 0.90; // Soglia attivazione HOLD (90% del target)
                     double criticalForceThreshold = 8.0;                      // Soglia minima di sicurezza per la presa
              // --- SPIEGAZIONE DELLA FUNZIONE hold() ---
              // Che cos'è? È un comando di mc_rtc che "congela" lo schiacciamento elastico virtuale accumulato.
              // Perché serve qui? Il braccio sinistro è molto morbido. Durante il movimento laterale, l'inerzia 
              // dell'oggetto o un piccolo ritardo cinematico potrebbero far muovere la traiettoria nominale più 
              // velocemente del robot reale. La molla virtuale (K=50) interpreterebbe questo errore geometrico 
              // tirando indietro il robot e staccandolo.
              // Con hold(true): Blocchiamo la deformazione al punto di contatto. Il robot smette di fuggire 
              // dal target geometrico in movimento e mantiene la forza di compressione stabile durante il moto.
                     if (lambdaFiltered_lowpass_cut >= targetForceThreshold) {
                                   // Se abbiamo raggiunto una buona forza di presa e l'HOLD è disattivato, lo attiviamo
                                   if (!leftImpedanceTask_->hold()) {
                                          mc_rtc::log::success("[Safety] Forza target raggiunta ({:.2f}N). Attivazione HOLD per bloccare le deformazioni elasteche!", 
                                                               lambdaFiltered_lowpass_cut);
                                          leftImpedanceTask_->hold(true);
                                          rightImpedanceTask_->hold(true);
                                   }
              // =========================================================================
              // 3. GESTIONE DELLA SICUREZZA E PROGRESSIONE DELLA TRAIETTORIA
              // =========================================================================
              if (lambdaFiltered_lowpass_cut < criticalForceThreshold) {
                     // CASO A: LA PRESA STA CEDENDO (Forza sotto la soglia critica)
                     
                     // Se eravamo in HOLD, lo rilasciamo immediatamente! Questo permette alla molla virtuale 
                     // di tornare attiva in modalità inseguimento per cercare di "avanzare" e recuperare il contatto.
                     if (leftImpedanceTask_->hold()) {
                            mc_rtc::log::warning("[Safety] Crollo della forza rilevato! Rilascio dell'HOLD per recuperare il contatto.");
                            leftImpedanceTask_->hold(false);
                            rightImpedanceTask_->hold(false);
                     }

                     // Congeliamo il tempo della traiettoria dell'oggetto: il movimento macroscopico si ferma 
                     // finché i bracci non ripristinano una presa sicura.
                     mc_rtc::log::warning("[Safety] Forza troppo bassa ({:.2f}N). Traiettoria congelata!", 
                                          lambdaFiltered_lowpass_cut);
              } else {
                     // CASO B: PRESA SICURA
                     // La forza è sopra la soglia critica, la traiettoria dell'oggetto può avanzare nel tempo
                     gains.collaborativeTime_ += timeStep;
              }
              
              // =========================================================================
              // 4. CALCOLO DELLE POSE DESIDERATE PER IL SOLUTORE QP
              // =========================================================================
              // Calcola la posa successiva dell'oggetto (profilo quintico + SLERP) basandosi sul tempo collaborativo
